...
\textbf{Exemplu:}
\[
  y^{''} = \arcsin x + \frac{x}{\sqrt{1 - x^2}} - \frac{\pi}{6}.
\]
\emph{Soluție:} Integrăm succesiv și obținem:
\begin{align*}
  y' &= x \arcsin x - \dfrac{\pi}{6} x + c_1 \\
...
\[
  x - e^{y^{''}} + y^{''} = 0.
\]
\emph{Soluție:} Putem nota $ y^{''} = t $ și atunci $ x(t) = e^t - t $.
Deoarece avem $ dy' = y^{''} dx $, rezultă:
\[
  dy' = t(e^t - 1) dt \Rightarrow y' = -\dfrac{t^2}{2} + te^t - e^t + c_1.
\]
...
\begin{enumerate}[(a)]
  \item Dacă ecuația este rezolvabilă prin funcții elementare în raport cu $ y^{(n)} $,
    atunci putem nota $ z = y^{(n-1)} $ și avem $ z' = f(z) $. Ecuația devine cu
  ...
  \item Dacă ecuația nu este rezolvabilă prin funcții elementare în raport cu
    ...
    \begin{align*}
      x &= \ds\int\frac{h'}{g} dt + c_1 \\
      y^{(n-1)} &= h(t) \\
      dy^{(n-2)} &= h(t) = \dfrac{hh'}{g} dt \\
      y^{(n-2)} &= \ds\int \frac{hh'}{g} dt + c_2 \\
                &\vdots \\
      y &= \int y' dx + c_n.
    \end{align*}
...
\textbf{Exemplu:} $ y^{''} = - \sqrt{1 - y^{'2}} $.
\emph{Soluție:} Facem schimbarea de funcție $ z(x) = y' $ și ecuația devine:
\[
...
\textbf{Exemplu:} $ (1 + x^2)y^{''} = 2xy' $.

\emph{Soluție:} Cu schimbarea de funcție $ y' = z(x) $, obținem ecuația:
\[
  \dfrac{z}{z'} = \dfrac{2x}{1 + x^2},
\]
iar apoi, prin integrare, rezultă $ z = y' = c_1(1 + x^2) $. În fine:
\[
  y(x) = c_1 x + c_1 \dfrac{x^3}{3} + c_2.
\]
\vspace{1cm}

\textbf{Exemplu 2:} $ y \cdot y^{''} - 2y^{'2} = 0 $.

\emph{Soluție:} Notăm $ y' = z $ și obținem:
\[
  y(y'z + yz') - 2y^2 z^2 = y^2(z^2 + z') - 2y^2 z^2 = 0,
\]
de unde rezultă $ y = 0 $ sau $ z' - z^2 = 0 $.

Din a doua variantă, deducem succesiv:
\[
  \dfrac{dz}{z^2} = dx \Rightarrow -\dfrac{1}{z} = x + c_1.
\]
Revenim la $ y(x) $ și avem:
\[
  - \dfrac{y}{y'} = x + c_1 \Rightarrow \dfrac{dy}{y} = -\dfrac{dx}{x + c_1} %
  \Rightarrow y(x) = \dfrac{c_2}{x + c_1}.
\]

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Ecuații de forma $ f(y', y^{''}, \dots, y^{(n)}) = 0 $}

Să remarcăm că astfel de ecuații nu conțin pe $ y $. Deci putem lua pe $ y' $
ca variabilă independentă și pe $ y^{''} $ ca fiind funcție de $ y' $.
Astfel, reducem discuția la un caz anterior.

\textbf{Exemplu:} $ x^2 y^{''} = y^{'2} - 2xy' + 2x^2 $.

\emph{Soluție:} Facem schimbarea de funcție $ y' = z(x) $ și obținem o
ecuație Riccati:
\[
  z' = \dfrac{z^2}{x^2} - 2 \cdot \dfrac{z}{x} + 2.
\]
...
\begin{align*}
  ...
  y'(x) &= x + \dfrac{c_1x}{x + c_1} \Rightarrow \\
...
$ y' = p $ și luăm pe $ y $ variabilă independentă.
...
\textbf{Exemplu:} $ 1 + y^{'2} = 2yy' $.
\emph{Soluție:} Putem lua $ y' = p $ drept funcție, iar pe $ y $ drept
variabilă independentă. Rezultă:
\[
  y^{''} = \dfrac{d}{dx} \Big( \dfrac{dy}{dx} \Big) = p \dfrac{dp}{dy}.
\]
...
\textbf{Exemplu 2:} $ y^{''} = y^{'2} = 2e^{-y} $.

\emph{Soluție:} Notăm $ y' = p $ și luăm ca necunoscută $ p = p(y) $.
Rezultă:
\[
  y^{''} = \dfrac{dy'}{dx} = p'p \Rightarrow p'p + p^2 = 2e^{-y}.
\]
...
a obține $ y' = \dfrac{dy}{dx} $ trebuie să aplicăm regula de derivare
a funcțiilor compuse. Folosind notația de la fizică $ \dot{y} = \dfrac{dy}{dt} $,
putem scrie:
\[
  \dfrac{dy}{dx} = \dfrac{dy}{dt} \cdot \dfrac{dt}{dx} = \dot{y} \cdot e^{-y}.
\]
Mai departe, de exemplu, pentru derivatele superioare:
\begin{align*}
  \dfrac{d^2 y}{dx^2} &= \dfrac{d}{dt} \Big( \dot{y} \cdot e^{-y} \Big) %
  \cdot \dfrac{dt}{dx} \\
                      &= e^{-2y} (\ddot{y} - \dot{y}).
\end{align*}
...
\begin{enumerate}[(a)]
  ...
  \item $ xy^{''} + 3y' = e^x $;
  \item $ x^3 y^{'''} + x^2y^{''} - 2xy' + 2y = \dfrac{1}{x^2} $.
\end{enumerate}
