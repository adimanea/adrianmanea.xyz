...
(pe care o vom înzestra cu structura de spațiu vectorial), iar $K$ va fi 
corpul de bază (al scalarilor), în practică luat cel mai adesea 
$K = \mathbb{R}$ sau $K = \mathbb{C}$.
...
O aplicație $f : K \times V \to V$, definită prin $(\alpha, x) \mapsto 
f(\alpha, x) \in V$ se numește \emph{lege de compoziție externă} pe $V$.
\end{definition}

Cu acestea, avem definiția fundamentală:
\begin{definition}\index{spațiu vectorial}
O mulțime nevidă $V$, înzestrată cu două legi de compoziție, una internă 
$+ : V \times V \to V$ și alta externă $\cdot : K \times V \to V$ se 
numește \emph{spațiu vectorial peste $K$} sau $K$-spațiu vectorial, 
notat pe scurt $_K V$ dacă sînt îndeplinite axiomele:
\begin{enumerate}[(1)]
...
\begin{enumerate}[(a)]
    ...
    \forall \alpha \in K, \forall x,y \in V$;
    \item $(\alpha + \beta) \cdot x = \alpha \cdot x + \beta \cdot x, 
    \forall \alpha, \beta \in K, \forall x \in V$;
    \item $\alpha \cdot (\beta \cdot x) = (\alpha \beta) \cdot x, 
    \forall \alpha,\beta \in K, \forall x \in V$;
    \item $1_K \cdot x = x, \forall x \in V$.
\end{enumerate}
\end{enumerate}

În acest context, elementele corpului $K$ se numesc \emph{scalari}, iar 
elementele din $V$ se numesc \emph{vectori}. Legea de compoziție externă se 
numește \emph{înmulțirea vectorilor cu scalari}. Dacă $K = \mathbb{R}$ 
...
între operațiile din corpul $K$, cele din $V$ și legea externă. Însă trebuie 
...
\emph{necomutative} $K$, caz în care înmulțirea dintre un vector și un 
...
\emph{spațiu vectorial stîng}, respectiv \emph{drept}, notate $_K V$ și 
$V_K$.
...
\begin{proposition}
Fie $V$ un $K$-spațiu vectorial. Au loc următoarele proprietăți:
\begin{enumerate}[(a)]
\item $(\alpha - \beta) \cdot x = \alpha \cdot x - \beta \cdot x, 
\forall \alpha, \beta \in K, \forall x \in V$;
\item $\alpha \cdot (x - y) = \alpha \cdot x - \alpha \cdot y, \forall 
\alpha \in K, \forall x, y \in V$;
\item $0_K \cdot x = 0_V, \forall x \in V$;
\item $(-\alpha) \cdot x = \alpha \cdot (-x) = - \alpha \cdot x, \forall 
\alpha \in K, \forall x \in V$;
\item Dacă $\alpha \cdot x = 0_V$, atunci $\alpha = 0_K$ sau $x = 0_V$.
\end{enumerate}
\end{proposition}
...
sînt următoarele:
\begin{example} \index{spațiu vectorial!exemple}
(1) $K$ este un spațiu vectorial peste el însuși, cu operațiile de corp, 
legea externă confundîndu-se cu cea internă.

(2) Mulțimea $K^n = K \times K \times \dots \times K 
= \{(x_1,\dots,x_n) \mid x_i \in K\}$, unde $K$ este un corp comutativ, 
este un spațiu vectorial peste $K$, numit \emph{spațiul aritmetic}, în 
raport cu legile de compoziție pe componente, pentru 
$x = (x_1,\dots,x_n), y = (y_1,\dots,y_n) \in K^n$:
\begin{align*}
+ : K^n \times K^n \to K^n, x + y 
&= (x_1 + y_1, x_2 + y_2, \dots, x_n + y_n); \\
\cdot : K \times K^n \to K^n, \alpha \cdot x 
&= (\alpha x_1, \alpha x_2, \dots, \alpha x_n).
\end{align*}

(3) Mulțimea matricelor $\mathcal{M}_{m,n}(K)$ este un spațiu vectorial 
peste $K$, operația internă fiind adunarea obișnuită a matricelor, iar 
...
vectorial peste corpul coeficienților acestui sistem, $K$. Soluțiile unui 
sistem cu $m$ ecuații și $n$ necunoscute pot fi privite ca elemente din 
$K^n$, care se adună și înmulțesc cu scalari respectînd operațiile din 
...
\index{spațiu vectorial!subspațiu}
Așadar, fie $V$ un $K$-spațiu vectorial și $W \seq V$ o submulțime nevidă.
\begin{definition}
$W$ se numește \emph{subspațiu vectorial} al lui $V$ dacă operațiile algebrice 
pe $V$ induc pe $W$ o structură de spațiu vectorial peste $K$.
...
vectoriale avem:
\begin{proposition}
Dacă $W$ este o submulțime nevidă a $K$-spațiului vectorial $V$, atunci 
următoarele afirmații sînt echivalente:
\begin{enumerate}[(1)]
\item $W \leq_K V$;
\item $\forall x,y \in W, \forall \alpha \in K$, avem $x + y \in W$ și 
$\alpha \cdot x \in W$;
\item $\forall x,y \in W, \forall \alpha, \beta \in K$, avem 
...
\[
W \leq_K V \Longleftrightarrow \alpha \cdot x + \beta \cdot y \in W, 
\forall x,y \in W, \alpha, \beta \in K.
\]
...
\emph{subspațiul nul}. De asemenea, $V \leq_K V$, iar aceste două exemple 
...
\end{definition}
\index{spațiu vectorial!suma subspațiilor}
\begin{proposition}
Fie $V$ un $K$-spațiu vectorial și $V_1, V_2$ două subspații ale sale. 
Atunci:
\begin{enumerate}[(a)]
\item $V_1 \cap V_2 \leq_K V$;
\item $V_1 + V_2 \leq_K V$.
\end{enumerate}
\end{proposition}
...
\begin{proposition}
Fie $V_1, V_2$ două subspații vectoriale ale $K$-spațiului $V$. Orice 
...
\begin{definition}
\index{spațiu vectorial!suma directă dde subspații}
Fie $V_1, V_2 \leq_K V$, cu $V_1 \cap V_2 = \{ 0_V \}$. Suma $V_1 + V_2$ se 
...
\begin{definition}
Fie $V$ un $K$-spațiu vectorial și $S = \{x_1, \dots, x_n\} \seq V$ o 
submulțime nevidă a lui $V$: Un vector de forma: 
\[
v = \alpha_1 x_1 + \alpha_2 x_2 + \dots + \alpha_n \cdot x_n, 
\alpha_i \in K, x_i \in S, \forall i
\] \index{spațiu vectorial!combinație liniară}
se numește \emph{combinație liniară} finită de elemente din $S$. Se notează, 
folosind terminologia engleză, $Sp(S)$\footnotemark sau, în unele cazuri, 
$\langle S \rangle_K$.
\end{definition}
\footnote{de la englezescul \emph{span}, adică acoperire, întindere}
...
preliminarii.
\begin{definition} \index{spațiu vectorial!vectori liniar independenți}
...
$\sum_i \alpha_i x_i = 0_V$, cu $\alpha_i \in K$ rezultă cu necesitate că 
toți $\alpha_i = 0$.
...
Cu acestea, avem:
\begin{proposition}
Fie $K$-spațiul vectorial $V$ și submulțimea 
...
\begin{definition}\index{spațiu vectorial!sistem de generatori}
...
\begin{proposition}
Fie $K$-spațiul vectorial $V$ și $S = \{x_1, \dots, x_n\} \seq V$ un 
...
\begin{theorem}[Teorema schimbului]\index{teorema!schimbului}
Fie $V$ un $K$-spațiu vectorial, $S = \{u_1, u_2, \dots, u_s\}$ un sistem 
...
\begin{definition}\index{spațiu vectorial!bază} \index{spațiu vectorial!dimensiune}
Fie $V$ un $K$-spațiu vectorial. Un sistem de vectori $\mathcal{B} \seq V$ 
se numește \emph{bază} în $V$ dacă $\mathcal{B}$ este simultan un sistem liniar 
independent și sistem de generatori pentru $V$.
\end{definition}

De exemplu, în spațiul aritmetic $K^n$, mulțimea $B = \{e_1, \dots, e_n\}$, 
unde $e_i$ este un șir de $n$ zerouri, cu elementul $1$ pe poziția $i$ este o 
bază, denumită \emph{baza canonică}. \index{spațiu vectorial!baza canonică}
...
În spațiul vectorial al matricelor de tip $(m, n)$ cu elemente din $K$, o 
...
\begin{proposition}
Orice două baze dintr-un $K$-spațiu vectorial finit dimensional au același 
număr de elemente.
\end{proposition}
...
\begin{definition} \index{spațiu vectorial!coordonatele unui vector}
Fie $V$ un $K$-spațiu vectorial și $B$ o bază a sa. Fie $x \in V$, scris în 
baza $B$ sub forma $x = \sum_i \alpha_i x_i$. Scalarii (unici) 
$\alpha_i \in K$ se numesc \emph{coordonatele} vectorului $x$ în baza $B$. 
Funcția bijectivă $f : V \to K^n$, care asociază unui vector coordonatele 
...
\begin{proposition}
Fie $V$ un $K$-spațiu vectorial de dimensiune $n$. Atunci:
\begin{enumerate}[(a)]
...
De asemenea, avem și:
\begin{theorem}
Fie $V$ un $K$-spațiu vectorial de dimensiune $n$ și 
...
\begin{theorem}[\textsc{H. Grassmann}] \index{teorema!Grassmann}
Dacă $V_1$ și $V_2$ sînt două subspații vectoriale ale $K$-spațiului 
vectorial finit dimensional $V$, atunci: 
\[
\dim_K V_1 + \dim_K V_2=\dim_K(V_1 + V_2) + \dim_K(V_1\cap V_2).
\]
\end{theorem}
...
\begin{definition}\label{def:matrice-apl-lin} \index{spațiu vectorial!matricea aplicației}
  Fie $V, W$ două $K$-spații vectoriale și fie $B_1 = \{ e_i \}_{i \in I}$
...
\index{spațiu vectorial!matricea de trecere}
Date mai multe baze ale unui spațiu vectorial, sîntem interesați de schimbarea
de coordonate ale vectorilor. Fie, pentru aceasta, un $K$-spațiu vectorial 
...
ordin $n$, $M = (c_{ij}) \in \mathcal{M}_n(K)$, transpusa coeficienților.
...
\begin{definition}\label{def:rang-defect}
  ...
  $\mathrm{Ker}(T)$.
\end{definition}
\index{teorema!rang-defect}
\begin{theorem}[Teorema rang-defect]\label{thm:rang-defect}
  Fie $V$ și $W$ două spații vectoriale peste același corp comutativ $K$,
  cu $\dim_K V = n$ și fie $T \in \kal{L}(V, W)$. Atunci:
  \[
...
11. Fie $V$ și $W$ două $K$-spații vectoriale. Dați exemplu de astfel de
spații și de o aplicație $f : V \to W$ care să fie morfism între grupurile
aditive $(V, +)$ și $(W, +)$, dar să nu fie $K$-liniară.
...
15. Dați exemplu de un $K$-spațiu vectorial $V$ și o submulțimea $W \seq V$
care să fie subgrup al lui $(V, +)$, dar să nu fie subspațiu vectorial al lui $V$.

\vspace{1cm}

16. Fie $V$ un $K$-spațiu vectorial și $S = \{v_1, \dots, v_n\}$ o mulțime
