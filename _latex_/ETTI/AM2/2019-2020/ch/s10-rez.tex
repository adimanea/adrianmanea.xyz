...
\begin{definition}\label{def:rez}\index{singularitate!reziduu}
...
\indent\indent 1. Să se determine discul de convergență pentru seriile:
\begin{enumerate}[(a)]
\item $ \ds\sum_{n \geq 0} \dfrac{(2n)!}{(n!)^2} (z - 3i)^n $;
\item $ \ds\sum_{n \geq 0} a^{n^2}z^n, |a| < 1 $;
\item $ \ds\sum_{n \geq 0} \dfrac{\cos in}{2^n} z^n $;
\item $ \ds\sum_{n \geq 1} \dfrac{\exp(in)}{n^2} (z - 1)^n $.
\end{enumerate}

\emph{Indicație:} Se folosește formula pentru raza de convergență.

\vspace{0.3cm}

2. Determinați dezvoltarea în serie Taylor sau Laurent pentru funcțiile următoare,
în jurul punctelor indicate $ z_0 $:
\begin{enumerate}[(a)]
...
3. Determinați punctele singulare și precizați natura lor pentru funcțiile:
\begin{enumerate}[(a)]
...
\item $ \ds\int_\gamma z^2 \exp\left(\dfrac{2z}{z+1}\right) dz $, unde
  $ \gamma: x^2 + y^2 + 2x = 0 $;
\item $ \ds\int_{|z - 1| = 3} \dfrac{iz + 1}{z^2 - iz + 2} dz $.
\end{enumerate}
...
\]
în jurul originii și în jurul punctelor $ z = \pm i $.
