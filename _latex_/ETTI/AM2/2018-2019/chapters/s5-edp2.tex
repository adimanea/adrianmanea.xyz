\chapter{Ecuații cu derivate parțiale de ordinul al doilea}
...
\textbf{Exerciții suplimentare (A.\ Negrescu)}

Să se afle soluția fiecăreia dintre următoarele ecuații diferențiale cu
derivate parțiale de ordinul al doilea:
\begin{enumerate}[(a)]
\item $ \begin{cases}
    u_{tt} &= 25 u_{xx} \\
    u(0, t) &= u(1, t) = 0 \\
    u(x, 0) &= x(1 - x) \\
    u_t(x, 0) &= 0 
  \end{cases}, \quad 0 < x < 1, t > 0 $
\item $ \begin{cases}
    u_{tt} &= 16 u_{xx} \\
    u(0, t) &= u(1, t) = 0 \\
    u(x, 0) &= \sin(5 \pi x) + 2 \sin(7 \pi x) \\
    u_t(x, 0) &= 0 
  \end{cases}, \quad 0 < x < 1, t > 0 $
\item $ \begin{cases}
    u_t &= 4 u_{xx} \\
