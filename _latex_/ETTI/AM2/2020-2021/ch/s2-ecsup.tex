...
\textbf{Exemplu:} $ x^2 y^{\prime\prime} + xy^{\prime} + y = x $.

\emph{Soluție:} Facem substituția $ |x| = e^t $ și, folosind calculele
de mai sus, obținem:
\[
  e^{2t} \cdot e^{-2t} (\ddot{y} - \dot{y}) + e^t \cdot e^{-t} \cdot \dot{y} + y = e^t.
\]
Echivalent: $ \ddot{y} + y = e^t $.

Scriem ecuația caracteristică, $ r^2 + 1 = 0 $, cu rădăcinile $ \pm i $, deci
sistemul fundamental de soluții este $ \{ \cos t, \sin t \} $ etc.
