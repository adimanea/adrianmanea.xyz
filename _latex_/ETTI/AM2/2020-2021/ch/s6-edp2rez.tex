...
\chapter{Rezolvarea EDP 2}

\section{Cazul coeficienților variabili}
...
\[
    (1 + x^2)\dfrac{\partial^2 u}{\partial x^2} + %
...
\begin{align*}
    \dfrac{\partial u}{\partial x} &= \dfrac{\partial u}{\partial \eta} \cdot %
    \dfrac{1}{\sqrt{1 + x^2}} \\
    \dfrac{\partial^2 u}{\partial x^2} &= \dfrac{1}{1 + x^2} \cdot \dfrac{\partial^2 u}{\partial \eta^2}%
    - \dfrac{x}{(1 + x^2)^{\dfrac{3}{2}}} \cdot %
    \dfrac{\partial u}{\partial \eta} \\
    \dfrac{\partial u}{\partial y} &= \dfrac{1}{\sqrt{1 + y^2}} \cdot \dfrac{\partial u}{\partial \tau} \\
    \dfrac{\partial^2 u}{\partial y^2} &= \dfrac{1}{1 + y^2} \cdot \dfrac{\partial^2 u}{\partial \tau^2}%
    - \dfrac{y}{(1 + y^2)^{\frac{3}{2}}} \cdot %
    \dfrac{\partial u}{\partial \tau}.
\end{align*}

Rezultă că ecuația se reduce la forma canonică:
\[
    \dfrac{\partial^2 u}{\partial \eta^2} + \dfrac{\partial^2 u}{\partial \tau^2} = 0.
\]
...
generală a ecuației:
\[
    x^2 \dfrac{\partial^2 u}{\partial x^2} - 2xy \dfrac{\partial^2 u}{\partial x\partial y} + %
    y^2 \dfrac{\partial^2 u}{\partial y^2} + x \dfrac{\partial u}{\partial x} + %
    y \dfrac{\partial u}{\partial y} = 0.
\]
...
\[
    x^2 \cdot \Big(\dfrac{dy}{dx}\Big)^2 + 2xy \cdot \dfrac{dy}{dx} + y^2 = 0 \Rightarrow %
    \dfrac{dy}{dx} = -\dfrac{y}{x} \Rightarrow xy = c.
\]
...
\begin{align*}
    \dfrac{\partial u}{\partial x} &= y\dfrac{\partial u}{\partial \tau} + %
    \dfrac{\partial u}{\partial \eta} \\
    \dfrac{\partial u}{\partial y} &= x \dfrac{\partial u}{\partial \tau} \\
    \dfrac{\partial^2 u}{\partial x^2} &= y^2 \dfrac{\partial^2 u}{\partial \tau^2} + %
    2y \dfrac{\partial^2 u}{\partial \tau\partial \eta} + %
    \dfrac{\partial^2 u}{\partial \eta^2} \\
    \dfrac{\partial^2 u}{\partial y^2} &= x^2 \dfrac{\partial^2 u}{\partial \tau^2} \\
    \dfrac{\partial^2 u}{\partial x\partial y} &= \dfrac{\partial u}{\partial \tau} + %
    xy \dfrac{\partial^2 u}{\partial \tau^2} + %
    x \dfrac{\partial^2 u}{\partial \tau \partial \eta}.
\end{align*}

Atunci ecuația devine:
\[
    \dfrac{\partial^2 u}{\partial \eta^2} + \dfrac{1}{\eta} \dfrac{\partial u}{\partial \eta} = 0.
\]
Ea se poate rescrie și rezolva astfel:
\[
    \dfrac{\partial}{\partial \eta} \Big( \eta \cdot \dfrac{\partial u}{\partial \eta} \Big) = 0 %
    \Rightarrow \eta \cdot \dfrac{\partial u}{\partial \eta} = f(\tau) \Rightarrow %
    \dfrac{\partial u}{\partial \eta} = \dfrac{1}{\eta} f(\tau).
\]
...
\[
    y \dfrac{\partial^2 u}{\partial x^2} + (x + y) \dfrac{\partial^2 u}{\partial x\partial y} + %
    x \dfrac{\partial^2 u}{\partial y^2} = 0.
\]

Deoarece $ A = y, B = \dfrac{x + y}{2}, C = x $, avem
\[
    \delta = AC - B^2 = \dfrac{-(x - y)^2}{4}
\]
...
\index{EDP!ordinul doi!coarda infinită}
\index{EDP!ordinul doi!d'Alembert}
...
\[
    a^2 \dfrac{\partial^2 u}{\partial x^2} = \dfrac{\partial^2 u}{\partial t^2}, %
    \forall x \in \RR, t > 0.
\]

Presupunem că avem condiții inițiale, astfel că problema devine o problemă Cauchy:
\[
    u(x, 0) = \varphi(x), \quad \dfrac{\partial u}{\partial t}(x, 0) = \psi(x), \forall x \in \RR,
\]
cu $ \varphi $ și $ \psi $ funcții date.

Cum ecuația este deja în forma canonică, asociem ecuația caracteristică:
\[
    a^2 dt^2 - dx^2 = 0 \Rightarrow \dfrac{dt}{dx} = \pm\dfrac{1}{a}.
\]
Rezultă că familiile de curbe caracteristice sînt:
\[
    \begin{cases}
...
\[
    \dfrac{\partial^2 u}{\partial \tau \partial \eta} = 0.
\]
Putem să o rezolvăm astfel:
\[
    \dfrac{\partial}{\partial \eta} \Big( \dfrac{\partial u}{\partial \tau} \Big) = 0 \Rightarrow %
    \dfrac{\partial u}{\partial \tau} = f(\tau).
\]
...
\[
    \theta_1(x) = \dfrac{\varphi(x)}{2} + \dfrac{1}{2a} \int_0^x \psi(\alpha) d\alpha + \dfrac{c}{2a},
\]
iar prin scădere, găsim:
\[
    \theta_2(x) = \dfrac{\varphi(x)}{2} - \dfrac{1}{2a} \int_0^x \psi(\alpha) d\alpha - \dfrac{c}{2a}.
\]
Revenind la variabilele inițiale, avem:
\[
    \begin{cases}
        \theta_1(x + at) &= \dfrac{\varphi(x + at)}{2} + %
        \dfrac{1}{2a} \int_0^{x + at} \psi(\alpha) d\alpha + \dfrac{c}{2a} \\
        \theta_2(x - at) &= \dfrac{\varphi(x - at)}{2} - %
        \dfrac{1}{2a} \int_0^{x - at} \psi(\alpha) d\alpha - \dfrac{c}{2a}
    \end{cases}.
\]
Putem asambla soluția finală în forma:
\begin{equation} \label{eq:dalembert}
    u(x, t) = \dfrac{1}{2} \big[ \varphi(x - at) + \varphi(x + at) \big] + %
    \dfrac{1}{2a} \int_{x - at}^{x + at} \psi(\alpha) d\alpha,
\end{equation}
...
\index{EDP!ordinul doi!coarda finită}
\index{EDP!ordinul doi!separarea variabilelor}
...
\[
    ...
    \dfrac{1}{a^2} \dfrac{T^{\prime\prime}}{T} = \dfrac{X^{\prime\prime}}{X}.
\]
...
\begin{itemize}
    ...
    \item Dacă $ \lambda > 0 $, soluția generală se scrie:
        ...
        $ \sin \sqrt{\lambda} l = 0 $, care înseamnă $ \lambda = \big(\dfrac{n\pi}{l}\big)^2 $.
        Putem scrie, atunci, soluția corespunzătoare acestei serii de valori în forma:
        \[
            X_n = c_n \sin \dfrac{n\pi}{l} x, \quad c_n \in \RR.
        \]
\end{itemize}

Înlocuim și integrăm acum ecuația după $ t $:
\[
    T^{\prime\prime} + a^2\Big( \dfrac{n\pi}{l} \Big)^2 T = 0,
\]
care are soluția generală:
\[
    T_n(t) = \alpha_n \cos \dfrac{n\pi}{l} at + \beta_n\sin \dfrac{n\pi}{l} at.
\]

Punînd laolaltă soluția după $ x $ și pe cea după $ t $, obținem:
\begin{equation} \label{eq:coarda-particulara}
    u_n(x, t) = X_n(x) T_n(t) = \Big( a_n \cos \dfrac{n\pi}{l} at + %
    b_n \sin \dfrac{n\pi}{l} at \Big) \cdot \sin \dfrac{n\pi}{l} x,
\end{equation}
...
\[
    \sum_{n = 1}^\infty \Big(a_n\cos\dfrac{n\pi}{l} at + %
    b_n \sin \dfrac{n\pi}{l} at\Big) \cdot \sin \dfrac{n \pi}{l} x.
\]
...
\[
    \begin{cases}
        u(x, 0) &= \sum_{n\geq 1} a_n \sin \dfrac{n\pi}{l} x = \varphi(x) \\
        \dfrac{\partial u}{\partial t}(x, 0) &= \dfrac{n\pi}{l}a \sum_{n \geq 1} b_n\sin\dfrac{n\pi}{l}x = \psi(x)
    \end{cases}.
\]
...
\begin{eqnarray} \label{eq:fourier}
    a_n &= \dfrac{2}{l} \ds\int_0^l \varphi(x) \sin \dfrac{n\pi}{l} xdx \\
    b_n &= \dfrac{2}{n\pi a} \ds\int_0^l \psi(x) \sin \dfrac{n \pi}{l} xdx.
\end{eqnarray}
...
\[
    ...
    \end{cases},
\]
iar forma canonică este $ \dfrac{\partial^2 u}{\partial \tau\partial \eta} = 0 $,
care are soluția:
\[
    u(\tau, \eta) = f(\tau) + g(\eta),
\]
...
\[
    \begin{cases}
        f(-3x) + g(x) &= 3x^2 \\
        -\dfrac{1}{3}f(-3x) + g(x) &= \sin x + c
    \end{cases}
\]
și prin schimbarea semnului primei ecuații și adunîndu-le, obținem:
\[
    \begin{cases}
        f(-3x) &= \dfrac{9}{4} x^2 - \dfrac{3}{4} \sin x - \dfrac{3c}{4} \\
        g(x) &= \dfrac{3}{4} x^2 + \dfrac{3}{4} \sin x + \dfrac{3c}{4}
    \end{cases}.
\]
Dacă notăm $ -3x = t $, atunci $ x = -\dfrac{t}{3} $ și găsim:
\[
    f(t) = \dfrac{t^2}{4} + \dfrac{3}{4} \sin \dfrac{t}{3} - \dfrac{3c}{4}.
\]
Așadar, soluția finală este:
\[
    \begin{cases}
        f(-3x + y) &= \dfrac{1}{4}(-3x + y)^2 + \dfrac{3}{4} \sin \dfrac{-3x + y}{3} - \dfrac{3c}{4} \\
        g(x + y)   &= \dfrac{3}{4}(x + y)^2 + \dfrac{3}{4} \sin(x + y) + \dfrac{3c}{4}
    \end{cases}.
\]
Rezultă că soluția problemei Cauchy este:
\[
    u(x, y) = \dfrac{1}{4}(-3x + y)^2 + \dfrac{3}{4} \sin \dfrac{-3x + y}{3} + %
    \dfrac{3}{4}(x + y)^2 + \dfrac{3}{4} \sin(x + y).
\]
...
\[
    ...
    \end{cases},
\]
care conduce la forma canonică $ \dfrac{\partial^2 u}{\partial \tau\partial \eta} = 0 $,
...
\[
    -\dfrac{1}{2} \varphi(2x) - 3 \psi(x) = \dfrac{2}{3}x^3 + k.
\]
Atunci:
\[
    \begin{cases}
        \varphi(2x) &= \dfrac{19}{96}(2x)^3 + c_1 \\
        \psi(x) &= -\dfrac{7}{12}x^3 - c_1
    \end{cases},
\]
de unde rezultă că soluția problemei Cauchy este:
\[
    u(x, y) = \dfrac{19}{96}(2x - y)^3 - \dfrac{7}{12}(x - 3y)^3.
\]

3. Rezolvați ecuația coardei vibrante infinite:
\[
    \dfrac{\partial^2 u}{\partial x^2} - \dfrac{\partial^2 u}{\partial t^2} = 0,
\]
...
\begin{align*}
    u(x, t) &= \dfrac{1}{2} \Big[ \dfrac{x - t}{1 + (x - t)^2} + \dfrac{x + t}{1 + (x + t)^2} \Big] + %
    \dfrac{1}{2} \int_{x - t}^{x + t} \sin y dy \\
            &= \dfrac{1}{2} \Big[ \dfrac{x - t}{1 + (x - t)^2} + \dfrac{x + t}{1 + (x + t)^2} \Big] - %
            \dfrac{1}{2}[\cos(x + t) - \cos(x - t)] \\
            &= \dfrac{1}{2} \Big[ \dfrac{x - t}{1 + (x - t)^2} + \dfrac{x + t}{1 + (x + t)^2} \Big] - %
            \dfrac{1}{2} \Big[ -2\sin \dfrac{x + t + x - t}{2} \sin \dfrac{x + t - x + t}{2} \Big] \\
            &= \Big[ \dfrac{x - t}{1 + (x - t)^2} + \dfrac{x + t}{1 + (x + t)^2} \Big] + \sin x \sin t.
\end{align*}
...
\[
    \varphi(x) = 4 \Big( x - \dfrac{x^2}{l} \Big),
\]
...
avem $ b_n = 0 $, iar
\[
    a_n = \dfrac{2}{l} \int_0^l 4\Big( x - \dfrac{x^2}{l} \Big) \sin \dfrac{n\pi}{l} xdx = %
    \dfrac{8}{l} \int_0^l x \sin \dfrac{n\pi}{l} xdx - %
    \dfrac{8}{l^2} \int_0^l x^2 \sin \dfrac{n\pi}{l}xdx.
\]

Calculăm:
\begin{align*}
    \int_0^l x\sin \dfrac{n\pi}{l} xdx &= -\dfrac{l}{n\pi} x \cos \dfrac{n\pi}{l}x \Big|_0^l + %
    \dfrac{l}{n\pi} \int_0^l \cos \dfrac{n\pi}{l} xdx \\
                                       &= -\dfrac{l^2}{n\pi}(-1)^n + %
                                       \dfrac{l^2}{n^2 \pi^2} \sin\dfrac{n\pi}{l} x\Big|_0^l \\
                                       &= (-1)^{n + 1} \dfrac{l^2}{n\pi}. \\
    \int_0^l x^2 \sin \dfrac{n\pi}{l} xdx &= -\dfrac{l}{n\pi}x^2 \cos \dfrac{n\pi}{l}x \Big|_0^l + %
    \dfrac{2l}{n\pi}\int_0^l x\cos \dfrac{n\pi}{l} xdx \\
                                          &= (-1)^{n + 1} \dfrac{l^3}{n\pi} + %
                                          \dfrac{2l}{n\pi}\Big[ \dfrac{l}{n\pi} x\sin \dfrac{n\pi}{l}x\Big|_0^l - \dfrac{l}{n\pi}\int_0^l \sin \dfrac{n\pi}{l}xdx \Big] \\
                                          &= (-1)^{n + 1}\dfrac{l^3}{n\pi} + %
                                          \dfrac{2l}{n^3 \pi^3}\cos \dfrac{n\pi}{l}x\Big|_0^l \\
                                          &= (-1)^{n + 1} \dfrac{l^3}{n\pi} + \dfrac{2l}{n^3 \pi^3}[(-1)^n + 1].
\end{align*}

Așadar:
\[
    a_n = (-1)^{n + 1} \dfrac{8l}{n\pi} - (-1)^{n + 1}\dfrac{8l}{n\pi} - %
    \dfrac{16}{n^3 \pi^3}[(-1)^n - 1],
\]
de unde obținem $ a_{2n} = 0, a_{2n+1} = \dfrac{32l}{(2n + 1)^3 \pi^3} $.

Punem laolaltă coeficienții și obținem soluția:
\[
    u(x, t) = \dfrac{32l}{\pi^3} \sum_{n \geq 0} \dfrac{1}{(2n + 1)^3} \cos\dfrac{(2n + 1)\pi}{l} t %
    \cdot \sin\dfrac{(2n + 1)\pi}{l} x.
\]
...
\[
    \dfrac{\partial^2 u}{\partial t^2} = 4 \cdot \dfrac{\partial^2 u}{\partial x^2}, %
    \quad 0 < x < \pi, t > 0,
\]
...
\[
    \begin{cases}
        ...
        \dfrac{\partial u}{\partial t}(x, 0) &= 2 \sin 4x + \sin 6x, 0 \leq x \leq \pi \\
...
Mai departe:
\[
    \dfrac{\partial u}{\partial t}(x, 0) = 2\sin 4x + \sin 6x \Rightarrow %
    \sum 2nb_n \sin nx = 2\sin 4x + \sin 6x.
\]
Egalînd coeficienții, avem: $ b_4 = \dfrac{1}{4}, b_6 = \dfrac{1}{12}, b_n = 0 $ în rest.

Rezultă:
\[
    u(x, t) = \cos 6t \sin 3x - 4\cos 20t \sin 10x + \dfrac{1}{4} \sin 8t \sin 4x + %
    \dfrac{1}{12} \sin 12t \sin 6x.
\]
...
\begin{enumerate}[(a)]
    \item
        \[
            \dfrac{\partial^2 u}{\partial x^2} = \dfrac{1}{9}\dfrac{\partial^2 u}{\partial t^2}, %
    ...
    \item
        \[
            \dfrac{\partial^2 u}{\partial x^2} - \dfrac{1}{4} \dfrac{\partial^2 u}{\partial t^2} = 0, %
    ...
    \item
        ...
        \[
            \begin{cases}
                u(x, 0) &= \sin 3x - 4\sin 10x \\
                \dfrac{\partial u}{\partial t}(x, 0) &= 2 \sin 4x + \sin 6x \\
                u(0, t) &= u(\pi, t) = 0
            \end{cases}.
        \]
\end{enumerate}
...
7. Să se determine soluția problemei Cauchy:
\[
    u_{xx} - \dfrac{1}{4} u_{tt} = 0, \quad x \in \RR, t \geq 0,
\]
cu condițiile inițiale:
\[
    u(x, 0) = e^x, \quad u_t(x, 0) = 4x.
\]

\emph{Indicație:} Metoda 1: Putem folosi direct formula lui D'Alembert. Avem
$ a = 2, f(x) = e^x, g(x) = 4x $, deci soluția se obține direct:
\[
    u(x, t) = \dfrac{1}{2} \Big( e^{x - 2t} + e^{x + 2t} \Big) + %
    \dfrac{1}{4} \int_{x - 2t}^{x + 2t} 4 \alpha d\alpha.
\]

Metoda 2: Alternativ, putem folosi rezolvarea directă. Scriem ecuația
atașată pentru $ t = t(x) $, care ne conduce la substituțiile:
\[
    \tau = x - 2t, \quad \eta = x + 2t.
\]
Forma canonică este $ u_{\tau\eta} = 0 $, a cărei soluție generală
este:
\[
    u(\tau, \eta) = f(\tau) + g(\eta).
\]

Folosim acum condițiile inițiale și determinăm $ f $ și $ g $, ca funcții
de $ x $ și $ t $.

\vspace{0.3cm}

8. Rezolvați problema Cauchy:
\[
    u_{xx} - \dfrac{1}{a^2} u_{tt} = 0, \quad x \in \RR, t \geq 0,
\]
cu condițiile inițiale:
\[
    u(x, 0) = \cos x, \quad u_t(x, 0) = 1.
\]
