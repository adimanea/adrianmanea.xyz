...
Mai trebuie să ne asigurăm de \emph{independența funcțională} a celor
două integrale prime. Astfel, dacă le notăm cu:
\[
  I_1(x, y, z) = x + y + z, \quad I_2(x, y, z) = x^2 + y^2 + z^2,
\]
...
\[
  \mathrm{rang} \begin{pmatrix}
    ...
    I_{1y} & I_{2y} \\
    I_{1z} & I_{2z}
  \end{pmatrix} = \mathrm{max} = 2, \quad \forall x, y, z \in \RR.
\]

(Am folosit notația cunoscută, $ I_{1x} = \dfrac{\partial I_1}{\partial x} $ etc.).

Într-adevăr, se vede imediat rezultatul, deoarece matricea de mai sus este
$  \begin{pmatrix}
    1 & 2x \\
    1 & 2y \\
    1 & 2z
  \end{pmatrix}
$
și putem lua orice minor, de pildă $ \begin{vmatrix} 1 & 2x \\ 1 & 2y \end{vmatrix} = 2y - 2x $,
care în general este nenul, cu excepția cazului $ y = x $.

Așadar, spunem că cele două integrale prime sînt funcțional
independente și am terminat.
...
\item $ \vec{v} = (y^2z^2, xyz^2, xy^2z) $;
\item $ \vec{v} = (xz, 2xz - yz, -x^2) $.
\end{enumerate}
...
2. Fie cîmpul vectorial:
\[
  \vec{V} = (x + y)\vec{i} + (y - x)\vec{j} - 2z\vec{k}.
\]

Determinați liniile de cîmp care conțin punctul $ M(1, 0, 1) $.

\vspace{0.3cm}

3. Rezolvați sistemele simetrice:
\begin{enumerate}[(a)]
    \item $ \dfrac{dx}{2y(2a - x)} = \dfrac{dy}{x^2 + z^2 - y^2 - 4ax} = \dfrac{dz}{-2yz} $,
        unde $ a \in \RR, x \neq 2a, y, z \neq 0 $;
    \item $ \dfrac{dx}{x(x + y)} = \dfrac{dy}{-y(x + y)} = \dfrac{dz}{(y - x)(2x + 2y + z)} $;
    \item $ \dfrac{dx}{1 + x^2} = \dfrac{dy}{x(2 - y)} = \dfrac{dz}{1 + z^2} $;
    \item $ \dfrac{dx}{x^2} = \dfrac{dy}{-xy} = \dfrac{dz}{y^2} $;
    \item $ \dfrac{dx}{x} = \dfrac{dy}{y} = \dfrac{zdz}{\sqrt{x^2 + y^2}} $;
    \item $ \dfrac{dx}{2y} = \dfrac{dy}{3x^2} = \dfrac{dz}{-6x^2y} $;
    \item $ \dfrac{dx}{2xz} = \dfrac{dy}{2yz} = \dfrac{dz}{z^2 - x^2 - y^2} $;
    \item $ \dfrac{dx}{2xz} = \dfrac{dy}{2yz} = \dfrac{dz}{x^2 - y^2} $.
\end{enumerate}

{
    \color{blue}
    \emph{Rezolvare (a):} Din primul și ultimul raport obținem:
    \[
        \dfrac{dx}{x - 2a} = \dfrac{dz}{z},
    \]
    care conduce la integrala primă $ \dfrac{x - 2a}{z} = c_1 $.

    Pentru a doua integrală primă, amplificăm primul raport cu $ x $,
    pe al doilea cu $ y $ și pe al treilea cu $ z $ și, prin egalare cu 
    ultimul raport obținem:
    \[
        \dfrac{xdx + ydy + zdz}{-y(x^2 + y^2 + z^2)} = \dfrac{dz}{-2yz} \Longrightarrow %
        \dfrac{d(x^2 + y^2 + z^2)}{x^2 + y^2 + z^2} = \dfrac{dz}{z},
    \]
    care conduce la integrala primă $ \dfrac{x^2 + y^2 + z^2}{z} = c_2 $.

    Se arată independența funcțională a celor două integrale prime etc.
}
...
\indent\indent Termenul în engleză pentru linii de cîmp este \texttt{(vector) field line}, iar cîteva
...
Vizualizarea grafică a liniilor de cîmp se poate face folosind GeoGebra,
accesînd link-ul de \href{https://www.geogebra.org/m/u3xregNW}{aici}.

De exemplu, reluăm primul exercițiu rezolvat:
\[
    \vec{V} = (z - y)\vec{i} + (x - z)\vec{j} + (y - x)\vec{k},
\]
unde am găsit liniile de cîmp:
\begin{align*}
    L_1 &: x + y + z = c_1 \\
    L_2 &: x^2 + y^2 + z^2 = c_2.
\end{align*}

Prima linie de cîmp este un plan, iar a doua este o sferă. Grafic, acestea
înseamnă că dacă reprezentăm în suficiente puncte din spațiu vectori
care alcătuiesc cîmpul $ \vec{V} $, vom vedea că apar \qq{tipare}, se disting
forme care reprezintă plane, respectiv sfere. Într-adevăr, reprezentarea
folosind GeoGebra pentru exemplul de mai sus arată, dintr-un punct de vedere,
plane la diverse distanțe față de origine, dar paralele,
iar din altul, sfere de raze diferite. Imaginea se poate obține interactiv,
pentru a putea fi rotită, introducînd componentele cîmpului în link-ul de
mai sus.

\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.5]{figs/vector-field2.png}
    \caption{Liniile de cîmp $ L_1 $, plane paralele}
\end{figure}


\begin{figure}[!htb]
    \centering
    \includegraphics[scale=0.5]{figs/vector-field1.png}
    \caption{Liniile de cîmp $ L_2 $, sfere concentrice}
\end{figure}
