...
liniară.
\begin{definition}\label{def:vvp} \index{vector!propriu} \index{valoare!proprie}
...
\begin{remark}\label{rk:diag-putere}
  ...
  ulterioare. În particular, dacă $ A = \dr{diag}(\lambda_i) $,
  atunci $ A^k = \dr{diag}(\lambda_i^k), \forall k $.
\end{remark}
