...
\chapter{Ecuații diferențiale de ordinul I}
...
Metoda generală de rezolvare este să folosim formula (am notat $ \exp(\square) = e^\square $):
\[
    y(x) = c \cdot \exp \left( -\int_{x_0}^x P(t) dt \right),
\]
...
avem:
\begin{itemize}
  \item Soluția particulară pentru varianta omogenă $y_p = c \cdot \exp\left( -\ds\int P(x) dx \right)$;
...
\]
de unde $\big( x + \varphi'(p) \big) \dfrac{dp}{dx} = 0$.
...
\subsection{Cu diferențiale totale}

\subsubsection{Rezolvare cu formulă}

Dacă are loc $\dfrac{\partial P}{\partial y} = \dfrac{\partial Q}{\partial x}$, atunci
ecuația se numește \emph{cu diferențiale totale}, iar soluția ei generală se prezintă prin:
\[
...
\begin{remark}\label{rk:partial}
  ...
  similar pentru $Q_x$. Deci condiția de diferențiale totale se mai scrie, pe scurt, $P_y = Q_x$.
\end{remark}
...
\emph{Soluție:} Remarcăm că avem $P(x, y) = 3x^2 - y$ și $Q(x, y) = 3y^2 - x$, deci
$P_y = Q_x = -1$, ecuația avînd diferențiale totale. Obținem direct, din formula
pentru soluția generală:
\[
...
\subsubsection{Rezolvare directă}

Din teorie, știm că dacă o ecuație diferențială are diferențiale totale, se poate
arăta că, în cazul ecuației scrisă în forma generală $ P(x, y) dx + Q(x, y) dy = 0 $,
există o funcție $ F(x, y) $ de clasă (cel puțin) $\kal{C}^1$ astfel încît
$ F_x = P(x, y) $ și $ F_y = Q(x, y) $.

Să vedem acest lucru pe exemplul de mai sus.
...
Avem $P(x,y) = 3x^2 - y $ și $ Q(x,y) = 3y^2 - x$. Am văzut că ecuația este
cu diferențiale totale, deci există o funcție $ F : \RR^2 \to \RR $ de clasă (cel puțin)
$ \kal{C}^1 $ astfel încît $ F_x = P $ și $ F_y = Q $. Deci:
\begin{align*}
  \dfrac{\partial F}{\partial x} &= 3x^2 - y \Rightarrow F(x, y) = \int 3x^2 - y dx = %
                                   x^3 + xy + c(y) \\
  \dfrac{\partial F}{\partial y} &= 3y^2 - x \Rightarrow F(x, y) = \int 3y^2 - x dy = %
                                   y^3 + xy + c(x),
\end{align*}
unde constantele de integrare pot să depindă, eventual, de variabilele în funcție
de care \emph{nu} se face integrarea.

Deoarece avem aceeași funcție $ F(x, y) $ pe care o căutăm, cele două expresii de mai
sus trebuie să coincidă, deci $ c(x) = x^3 $, iar $ c(y) = y^3 $. În final:
\[
  F(x, y) = x^3 + y^3 - xy + k, \quad k \in \RR
\]
este funcția căutată, care dă \emph{soluția implicită} a ecuației.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Cu factor integrant}

Dacă ecuația nu are diferențiale totale, se caută \emph{un factor integrant},
adică o funcție $\mu(x, y)$ cu care se înmulțește ecuația pentru a deveni
cu diferențiale totale.
...
\]
care este cu diferențiale totale. Atunci putem scrie direct soluția generală:
\[
...
\begin{enumerate}[(1)]
  \item $xy^\prime - y + 2x^2 y^2 = 0 $; \hfill(Bernoulli)
  \item $(2x - y + 2) dx + (-x + y + 1) dy = 0 $; \hfill(exactă)
  \item $ xy^\prime = 4y + x^2 \sqrt{y} $; \hfill(Bernoulli)
  \item $ (x^2 + 1) y^\prime + 2xy^2 = 0 $; \hfill(exactă)
  \item $ (2x + y + 1) dx + (x + y - 1) dy = 0 $; \hfill(exactă)
  \item $ y^\prime = \dfrac{x^2}{y} $; \hfill(variabile separabile)
  \item $ y^\prime = \dfrac{y}{x} + x $; \hfill(liniară, neomogenă)
  \item $ 3x^2 y dx - (\ln y + x^3) dy = 0 $; \hfill(exactă)
  \item $ y^\prime = x^2 y $; \hfill(variabile separabile)
  \item $ y^\prime = \dfrac{x^2 + y^2}{2xy} $; \hfill(Bernoulli)
  \item $ y^\prime = 2xy^2 $; \hfill(variabile separabile)
  \item $ y^\prime = \dfrac{4x}{y^2} $; \hfill(variabile separabile)
  \item $ (x + y + 1)dx + (x - y^3 + 3) dy = 0 $; \hfill(exactă)
  \item $ y^\prime = \dfrac{1}{x}y - \dfrac{\ln x}{x} $; \hfill(liniară, neomogenă)
  \item $ y^\prime = -\dfrac{y}{x} + \dfrac{e^x}{x} $; \hfill(liniară, neomogenă)
  \item $ (x + y^2) dx + y(1 - x)dy = 0 $; \hfill(exactă)
  \item $ 3x^2 y dx - (x^3 + \ln y) dy = 0 $; \hfill(exactă)
  \item $ y^\prime = -2y + x^2 + 2x $; \hfill(liniară, neomogenă)
  \item $ y^\prime = \dfrac{2y}{x} + \dfrac{3}{x^2} $; \hfill(liniară, neomogenă)
  \item $ y^\prime = 2xy + x^3 y^{\frac{1}{2}} $; \hfill(Bernoulli)
  \item $ y^\prime = -\dfrac{1}{x}y + \dfrac{\ln x}{x} y^2 $. \hfill(Bernoulli)
\end{enumerate}
