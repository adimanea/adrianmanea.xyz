...
\subsection{Rezolvare cu formulă}
...
\subsection{Rezolvare directă}

Din teorie, știm că dacă o ecuație diferențială este exactă, ea se mai numește
\emph{cu diferențiale totale}, deoarece se poate arăta că, în cazul ecuației
scrisă în forma generală $ P(x, y) dx + Q(x, y) dy = 0 $, există o funcție
$ F(x, y) $ de clasă (cel puțin $\kal{C}^1$) astfel încît $ F_x = P(x, y) $ și
$ F_y = Q(x, y) $.
...
exactă, deci există o funcție $ F : \RR^2 \to \RR $ de clasă (cel puțin)
...
\begin{enumerate}[(1)]
  ...
  \item $ (3x^2 + 2)y^\prime + xy^2 = 0 $; \hfill(exactă)
