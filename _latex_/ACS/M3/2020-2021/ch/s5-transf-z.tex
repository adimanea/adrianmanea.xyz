...
\begin{enumerate}[(1)]
    \item \textbf{Liniaritatea:} Transformata $ Z $ este liniară în raport
    cu înmulțirea cu scalari și adunarea semnalelor discrete;
    \item \textbf{Inversarea transformării $ Z $:} Fie $ s \in S_d^+ $, cu $ s = (a_n) $.
        Presupunem că $ L_s(z) $ este olomorfă în domeniul $ |z| \in (r, R) $. Atunci
        putem recupera semnalul $ a_n $ prin formula:
        \[
            a_n = \dfrac{1}{2 \pi i} \int_{\gamma} z^{n-1} L_s(z) dz, \quad n \in \ZZ,
        \]
...
dată de:
\begin{enumerate}[(a)]
    \item $L_s (z) = \dfrac{z}{(z-3)^2}$ ;
    \item $L_s (z) = \dfrac{z}{(z-1)(z^2 + 1)}$ ;
    \item $L_s (z) = \dfrac{z}{(z-1)^2(z^2 + z - 6)}$ ;
    \item $L_s (z) = \dfrac{z}{z^2 + 2az + 2a^2}, a > 0$ parametru.
\end{enumerate}
...
(a) Avem:
\begin{align*}
    x_n &= \dfrac{1}{2\pi i} \int_{|z| = \rho} z^{n-1} L_s (z) dz \\
        &= \Rez(z^{n-1} L_s(z), 3) \\
...
\begin{align*}
    x_n &= \dfrac{1}{2 \pi i} \int_{|z| = \rho} z^{n-1} L_s(z) dz \\
        &= \Rez(z^{n-1} L_s (z), 1) + \Rez(z^{n-1} L_s(z), i) + 
        \Rez(z^{n-1} L_s(z), -i) \\
    \Rez(z^{n-1}L_s(z),1) &= \lim_{z \to 1} z^{n-1} \dfrac{z}{(z - 1)(z^2 + 1)} 
    \cdot (z-1) = \dfrac{1}{2} \\
    \Rez(z^{n-1} L_s(z), i) &= \dfrac{i^n}{2i \cdot (i-1)} \\
    \Rez(z^{n-1} L_s(z), -i) &= \dfrac{(-1)^n i^n}{2i(i+1)}.
\end{align*}
...
(c)
\begin{align*}
    \Rez(z^{n-1} L_s(z), 1) &= \lim_{z\to 1} \Big[ z^{n-1} \cdot (z-1)^2 \cdot %
    \dfrac{z^2}{(z-1)^2 \cdot (z^2 + z - 6)} \Big]^{\prime}\\
                            &= - \dfrac{4n + 3}{16}. \\
    \Rez(z^{n-1} L_s(z), 2) &= \dfrac{2^n}{5} \\
    \Rez(z^{n-1} L_s(z), -3) &= -\dfrac{(-3)^n}{80}.
\end{align*}
...
Să se arate că $Y(z) = \dfrac{z}{z-1} X(z)$, unde $ Y(z) = L_x(z) $ și $ X(z) = L_x(z) $
sînt transformatele Z pentru semnalele $ x $ și $ y $.:
...
\[
    L_x(z) \cdot (z^2 - z - 1) = z \Longrightarrow x_n 
...
\[
    L_x(z) \cdot (z^2 - z + 1) = z \Longrightarrow L_x(z) 
    = \dfrac{z}{z^2 - z + 1}.
\]
...
$L_x(z) = - \dfrac{z(z+2)}{(z^2 + z + 1)^2}$. Descompunem în fracții 
...
Aplicăm transformata $Z$ și obținem:
\begin{align*}
    z L_x(z) + 3 L_x(z) &= \dfrac{z}{z-1} + 2z \\
                        &= \dfrac{2z^2 + 3z}{z-1} \\
    \Longrightarrow L_x(z) &= \dfrac{2z^2 + 3z}{(z-1)(z+3)} \\
...
\begin{align*}
    L_{s_1}(z) &= -z L_{s_2^{\prime}}(z) 
    = - z \big( \dfrac{z}{z-4} \big)^{\prime} = \dfrac{4z}{(z-4)^2} \\
    \Longrightarrow L_x(z)(z^2 - 4z + 3) 
               &= \dfrac{4z}{(z-4)^2} + \dfrac{z}{z-4} + z \\
               &= \dfrac{z^2}{(z-4)^2} + z \\
    \Longrightarrow  L_x(z) &= \dfrac{z(z^2 - 7z + 16)}{(z-4)^2 (z-1) (z-3)}.
\end{align*}
...
putem obține:
\[
    L_{x_{n+2}}(z) - 2L_{x_{n + 1}}(z) + L_{x_n}(z) = 2L_1(z),
\]
iar $ L_{x_{n+2}}(z) = L_{x_n \ast \delta_{-2}}(z) = L_{x_n}(z) \cdot L_{\delta_{-2}}(z) $ etc.

(2) Se poate aplica teorema de deplasare. În aceeași recurență de mai sus,
de exemplu, avem:
\[
    L_{x_{n+2}}(z) = z^n \left( L_{x_n}(z) - x_0 - x_1 z^{-1} \right)
\]
și la fel pentru celelalte.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Exerciții suplimentare (L.\ Costache)}

1. Să se determine semnalul $ x \in \S_d^+ $ a cărui transformată $ Z $ este:
\begin{enumerate}[(a)]
    \item $ L_x(z) = \dfrac{2z+3}{z^2 - 5z + 6} $;
    \item $ L_x(z) = \dfrac{z^2 + 1}{z^2 - z + 1} $;
    \item $ L_x(z) = \dfrac{z}{(z - 5)^2} $;
    \item $ L_x(z) = \dfrac{z + 2}{z^2 + z + 1} $.
\end{enumerate}

\vspace{0.3cm}

2. Cu ajutorul transformatei $ Z $, să se determine șirurile $ (x_n)_{n \in \NN} $
definite prin următoarele relații de recurență:

\begin{enumerate}[(a)]
    \item $ x_0 = 4, x_1 = 6 $ și $ x_{n+2} - 3x_{n+1} + 2x_n = 0, \forall n \in \NN $;
    \item $ x_0 = 0, x_1 = 11, x_2 = -8, x_3 = 6 $ și $ x_{n+4} - \dfrac{5}{2} x_{n+3} + \dfrac{5}{2} x_{n+1} - x_n = 1, \forall n \in \NN $;
    \item $ x_0 = 0, x_1 = 3 $ și $ x_{n+2} - 4x_{n+1} + 3x_n = 2, \forall n \in \NN $;
    \item $ x_0 = 0, x_1 = -1 $ și $ x_{n + 2} + x_{n + 1} - 6x_n = n, \forall n \in \NN $;
    \item $ x_0 = 0, x_1 = 1 $ și $ x_{n + 2} - 5x_{n+1} + 6x_n = 4 \cdot 5^n, \forall n \in \NN $.
\end{enumerate}