...
\lhead{\small \color{gray} BCD-Aero (B), 2017---2018, L. Costache}
...
\begin{center}
  {\large\textbf{Seminar 10 \\ Funcții de mai multe variabile}}
\end{center}
...
Să se calculeze, folosind definiția, derivata după direcția
$h = \big( \frac{1}{3}, \frac{2}{3}, \frac{2}{3} \big)$ a funcției $f$ în punctul
$(x, y, z) = (1, 1, 2)$.
...
5. Să se cerceteze limitele iterate și limita globală în origine pentru
funcțiile:
\begin{enumerate}[(a)]
\item $f(x, y) = \frac{x - y + x^2 + y^2}{x + y}, x + y \neq 0$;
\item $f(x, y) = x \cdot \sin\frac{1}{y}, y \neq 0$;
\item $f(x, y) = (x + y) \sin \frac{1}{x} \sin \frac{1}{y}, x, y \neq 0$;
\item $f(x, y) = \frac{x^2 y^2}{x^2 y^2 + (x - y)^4}$.
\end{enumerate}

\emph{Indicații:}
\begin{enumerate}[(a)]
\item Limitele iterate sînt diferite. Pentru limita globală, putem lua $y=mx$;
\item Limitele iterate sînt diferite (una nu există, una este nulă). Pentru
  limita globală, putem verifica cu definiția, datorită mărginirii $\sin \frac{1}{y}$;
\item Limitele iterate nu există, dar limita globală e nulă;
\item Limitele iterate sînt nule, dar limita globală nu există ($y = mx$, depinde de $m$).
\end{enumerate}

\vspace{1cm}

6. Să se arate că funcția:
\[
  f(x, y) =
  \begin{cases}
    \frac{2xy}{x^2 + y^2}, & (x, y) \neq (0, 0) \\
  ...
  \end{cases}
\]
este continuă în raport cu fiecare variabilă în parte, dar nu este continuă
în raport cu ansamblul variabilelor.

\emph{Indicație:} Fixăm $y = b$, iar funcția este continuă după $x$. Fixăm
$x = a$, iar funcția este continuă după $y$.

În origine, pentru $y = mx$, limita nu există.
...
Să se arate că $f$ este de clasă $\kal{C}^1$ pe $\RR^2$.

\emph{Indicație:} Arătați că derivatele parțiale ale lui $f$ sînt funcții
continue pe $\RR^2$.
...
{\color{red}
  \textbf{Calculul derivatei parțiale cu definiția:}

  Fie funcția:
  \[
    f(x, y) = 3x^2 - 2xy + 5y^2 - 3x + 2.
  \]

  Vrem să calculăm $\frac{\partial f}{\partial x}(1, 2)$. Deoarece avem de calculat
  derivata parțială \emph{în raport cu $x$}, putem deja să evaluăm funcția în $y = 2$.
  Altfel spus, a calcula derivata parțială în raport cu $x$ în punctul $(1, 2)$ este
  echivalent cu a calcula derivata funcției $f(x, 2)$, în $x = 1$. Iar aceasta este
  o problemă de clasa a XI-a, fiind vorba despre o funcție de o variabilă.

  Avem, așadar:
  \[
    f(x, 2) = 3x^2 - 7x + 22 = g(x).
  \]
  Și mai departe, calculăm derivata în general, pe care apoi o evaluăm în $x = 1$:
  \[
    g'(x_0) = \ds\lim_{x \to x_0} \frac{g(x) - g(x_0)}{x - x_0} = %
    \ds\lim_{x \to x_0} \frac{3(x^2 - x_0^2) - 7(x - x_0)}{x - x_0} = 6x_0 - 7.
  \]

  Atunci, $g'(1) = -1 = \frac{\partial f}{\partial x}(1, 2)$.
}
