...
\item Determinăm \emph{valorile proprii}\footnotemark $ \lambda_{1,2} $ ale matricei hessiene,
...
\end{enumerate}
\footnotetext{Există o metodă alternativă, aceea care se bazează pe
    \emph{criteriul lui Sylvester} pentru forme pătratice, studiată
    la curs. Cîteva indicații teoretice puteți găsi și în notițele
    mele \href{https://adrianmanea.xyz/docs/17-18-fia-bcd/seminar11a-extreme-alt.pdf}{aici}.

    Vom prefera, în seminar, să folosim această metodă a valorilor
proprii, însă.}
...
și o vom exemplifica în seminarul următor.
...
\indent\indent 1. Determinați punctele de extrem și valorile extreme pentru funcțiile:
\begin{enumerate}[(a)]
\item $ f : D \seq \RR^2 \to \RR, f(x, y) = x^2 + xy + y^2 - 4 \ln x - 10\ln y + 3 $;
\item $ f(x, y) = 3xy^2 - x^3 - 15x - 36y + 9 $, definită (i) pe $ \RR^2 $
...
\item $ f(x, y, z) = x^2 + y^2 + z^2 + 2x + 6y - 6z $, definită pe $ \RR^3 $;
\item $ f(x, y, z) = x^2 + y^2 + z^2 - 6x + y - 2z $, definită pe $ \RR^3 $;
\item $ f(x, y, z) = \dfrac{1}{x} + \dfrac{x}{z} + \dfrac{y}{z} + z $, definită pe $ \RR^3 \setminus \{ (0,0,0) \} $.
\end{enumerate}
 
În toate exercițiile de mai sus este recomandabil să reprezentați
grafic domeniile de definiție atunci cînd sînt mărginite.
...
4. Fie funcția: $ f : D \seq \RR^2 \to \RR, f(x, y) = x^2 + 2xy - 4x - y $.
