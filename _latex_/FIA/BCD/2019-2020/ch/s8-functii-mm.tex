...
\begin{enumerate}[(a)]
\item $f(x, y) = \dfrac{2xy}{x^2 + y^2}, (x, y) \neq (0, 0)$;
\item $f(x, y) = \dfrac{y^2 + 2x}{y^2 - 2x}, (x, y) \neq (0, 0)$.
\end{enumerate}
...
\[
  f(x, y) = \begin{cases}
    \dfrac{xy^2 + \sin(x^3 + y^5)}{x^2 + y^4}, & (x, y) \neq (0, 0) \\
    & \\
    0, & (x, y) = (0, 0)
  \end{cases}
\]
...
\[
  f(x, y) = \begin{cases}
    \dfrac{xy + x^2 y \ln |x + y|}{x^2 + y^2}, & (x, y) \neq (0, 0) \\
    0, & (x, y) = (0, 0)
  \end{cases}
\]
...
\indent \indent 0. Calculați următoarele derivate parțiale folosind definiția:
\begin{enumerate}[(a)]
...
7. Fie funcția $ f(x, y) = xy\varphi\left(\ln(x + y) + \dfrac{x}{y}\right) $,
unde $ \varphi : \RR \to \RR $ este o funcție derivabilă.

Aduceți la forma cea mai simplă expresia:
\[
  E(x, y) = x\dfrac{\partial f}{\partial x} + y \dfrac{\partial f}{\partial y} - %
  \dfrac{x + y}{xy} f(x, y).
\]
